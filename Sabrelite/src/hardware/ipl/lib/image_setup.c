...
int image_setup(const paddr_t addr)
{
    paddr_t ram_addr;
    ...
    startup_hdr.imagefs_paddr = (uint32_t)addr + startup_hdr.startup_size - startup_hdr.paddr_bias;

    //
    // Copy startup to ram_addr.
    //

    copy(ram_addr, (paddr_t)(&startup_hdr), sizeof(startup_hdr));
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/hardware/ipl/lib/image_setup.c $ $Rev: 975396 $")
#endif
