...
next_offset:
			...
			if((tag = hwi_tag_find(hwi_off, HWI_TAG_NAME_regname, &tag_idx)) != NULL) {
				reg_name_string =  __hwi_find_string(tag->regname.regname);
				if(strcmp("value", reg_name_string) == 0) {
					...
					write_reg[num_writes][REG_VAL] |= ((uint32_t)tag->regname.offset << 16);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/support/wdtkick/main.c $ $Rev: 982757 $")
#endif
