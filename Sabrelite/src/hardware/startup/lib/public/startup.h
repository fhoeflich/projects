/*
 * Copyright 2014,2022,2023, BlackBerry Limited.
 *
...
#define PROCESSORS_MAX	64	// set this to match kernel (at least a search will find this now)
...
#include <x86_64/cpu_startup.h>
#elif defined(__aarch64__)
#include <aarch64/cpu_startup.h>
#endif
#include <sys/restore_ifs.h>
...
#define COMMON_OPTIONS_STRING   CPU_COMMON_OPTIONS_STRING "ACc:D:F:f:I:i:K:M:N:o:P:R:S:Tvr:j:ZH"
...
struct local_syspage {
	...
	SYSPAGE_SECTION(smp);
    SYSPAGE_SECTION(hypinfo);
    SYSPAGE_SECTION(cluster);
...
void acpi_spcr_parse(struct debug_device *dbgdev);
...
#define AS_ATTR_RAM	AS_ATTR_NONE
#define AS_ATTR_ROM	AS_ATTR_NONE
#define AS_ATTR_DEV	AS_ATTR_NONE
...
/*
 * AARCH64 version of u-boot stashes the FDT physical address in x0 (r2 in AARCH32)
 * Then, its register value gets copied to boot_regs[0] (boot_regs[2] in AARCH32)
 */
 ...
 * ------------------------------------------------------------------
 */
#define HYP_FLAG_INVALID         (-1)
#define HYP_FLAG_DISABLED        (0)
#define HYP_FLAG_ENABLED         (1)
#define HYP_FLAG_CPU_SPECIFIC_1  (2)
#define HYP_FLAG_CPU_SPECIFIC_2  (4)
// See cpu_startup.h for additional HYP_FLAG_ values

typedef int hyp_flags_t;  // Holds HYP_FLAG_ values

void         hypervisor_init(unsigned  cpunum);
void         hypervisor_set_options(hyp_flags_t  flags);
hyp_flags_t  hypervisor_get_required_flags(void);
void         hypervisor_syspage_init(void);
hyp_flags_t  arch_hypervisor_validate_flags(hyp_flags_t  option_flags);
void         arch_hypervisor_init(unsigned  cpunum, hyp_flags_t  hyp_flags);
...
#define FDT_BOOTOPT_ADDRPASS_BY_REG    0x00000001
#define FDT_BOOTOPT_ADDRPASS_BY_ARGS   0x00000002
extern int							fdt_boot_option;
