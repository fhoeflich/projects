 ...
 *                     Otherwise --failure
 */
int32_t f3s_snor_suspend(f3s_dbase_t *dbase, f3s_access_t *access, uint32_t flags, uint32_t offset)
{
    snor_chip_t *chip;
    snor_ctrl_t *ctrl;
    ...
    // For now, supported only if F-status register is available.
    if (!(chip->dcaps & SNOR_DCAPS_FSTATUS)) return (ENOTSUP);
    ...
    for (loop = 1000; loop > 0; loop--) {
        if (snor_read_register(chip, SNOR_CMD_RDFSR, 0, 0, sr, nsr) != EOK) return (EIO);
        ...
        sus = 0;
        for (i = 0; i < nsr; i++) {
            if (sr[i] & SNOR_FLGSTS_RDY) {
                rdy++;
                if (sr[i] & SNOR_FLGSTS_ESUS) {
                    sus++;
                }
            }
        }
