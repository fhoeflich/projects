...
MTDFLASH_ROOT=$(PRODUCT_ROOT)/$(MTD_FLASH)
endif
...
-include $(PRODUCT_ROOT)/boards/$(SECTION)/extra.mk

# Check for USEFILE override

GENERIC_USEFILE=$(MTDFLASH_ROOT)/usagev$(fs_ver).use
SOC_USEFILE=$(MTDFLASH_ROOT)/soc/$(SOC_NAME)/$(SOC_NAME).use
BOARD_USEFILE=$(PRODUCT_ROOT)/boards/$(SECTION)/$(SECTION).use
USEFILE=$(GENERIC_USEFILE)

ifneq ($(wildcard $(SOC_USEFILE)),)
USEFILE=$(PWD)/devf-$(SECTION).use
$(shell cat $(GENERIC_USEFILE) > $(USEFILE))
$(shell cat $(SOC_USEFILE) >> $(USEFILE))

ifneq ($(wildcard $(BOARD_USEFILE)),)
$(shell cat $(BOARD_USEFILE) >> $(USEFILE))
endif

EXTRA_CLEAN += devf-$(SECTION).use
endif
