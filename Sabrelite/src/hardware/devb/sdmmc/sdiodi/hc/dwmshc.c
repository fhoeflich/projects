...
static int dw_clk( sdio_hc_t *hc, int clk )
{
	...
	for( clkd = 0, div = 1; clkd <= 0xFF; ) {
		if( ( ( msh->sclk / msh->divratio ) / div ) <= (uint64_t)clk ) {
			break;
		}
		div = ++clkd * 2;
	}
...
int dw_init( sdio_hc_t *hc )
{
	...
	msh->sclk = (cfg->clk) ? cfg->clk : DW_SCLK;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/hardware/devb/sdmmc/sdiodi/hc/dwmshc.c $ $Rev: 982571 $")
#endif
