...
static sdio_hc_entry_t sdhci_dwc_hc_entry = {	.nentries			= SDIO_HC_ENTRY_NFUNCS,
												.dinit				= sdhci_dwc_dinit,
												.pm					= NULL,
												.cmd				= sdhci_cmd,
												.abort				= sdhci_abort,
												.event				= sdhci_event,
												...
												.pwr				= sdhci_pwr,
												.clk				= sdhci_dwc_clk,
												.bus_mode			= sdhci_bus_mode,
												.bus_width			= sdhci_bus_width,
												.timing				= sdhci_dwc_timing,
												.signal_voltage		= sdhci_signal_voltage,
												.drv_type			= sdhci_drv_type,
...
#endif
        if( ( sts & SDHCI_INTR_DTO ) ) {
            cs = CS_DATA_TO_ERR;
        }
        if( ( sts & SDHCI_INTR_DCRC ) ) {
            cs = CS_DATA_CRC_ERR;
        }
        if( ( sts & SDHCI_INTR_DEB ) ) {
            cs = CS_DATA_END_ERR;
        }
        if( ( sts & SDHCI_INTR_CTO ) ) {
            cs = CS_CMD_TO_ERR;
        }
        if( ( sts & SDHCI_INTR_CCRC ) ) {
            cs = CS_CMD_CRC_ERR;
        }
        if( ( sts & SDHCI_INTR_CEB ) ) {
            cs = CS_CMD_END_ERR;
        }
        if( ( sts & SDHCI_INTR_CIE ) ) {
            cs = CS_CMD_IDX_ERR;
        }
...
static int sdhci_pwr( sdio_hc_t *hc, int vdd )
{
    ...
    if( vdd ) {
        ...
        switch( vdd ) {
            case OCR_VDD_17_195:
                hctl |= SDHCI_HCTL_SDVS1V8;
                break;

            case OCR_VDD_30_31:
            case OCR_VDD_29_30:
                hctl |= SDHCI_HCTL_SDVS3V0;
                break;

            case OCR_VDD_33_34:
            case OCR_VDD_32_33:
                hctl |= SDHCI_HCTL_SDVS3V3;
                break;
...
static int sdhci_drv_type( sdio_hc_t *hc, int type )
{
    ...
    if( !( hctl2 & SDHCI_HCTL2_PRESET_VAL ) ) {
        switch( type ) {
            case DRV_TYPE_A:
                hctl2 |= SDHCI_HCTL2_DRV_TYPE_A;
                break;
            case DRV_TYPE_C:
                hctl2 |= SDHCI_HCTL2_DRV_TYPE_C;
                break;
            case DRV_TYPE_D:
                hctl2 |= SDHCI_HCTL2_DRV_TYPE_D;
                break;
            case DRV_TYPE_B:
            default:
                hctl2 |= SDHCI_HCTL2_DRV_TYPE_B;
                break;
        }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/hardware/devb/sdmmc/sdiodi/hc/sdhci_dwc.c $ $Rev: 982571 $")
#endif
