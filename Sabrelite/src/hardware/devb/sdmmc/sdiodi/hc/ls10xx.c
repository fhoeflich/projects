...
static int ls10xx_intr_event(sdio_hc_t *hc)
{
    ...
    if(sts & LS10XX_IS_ERRI) {
        ...
        if(sts & LS10XX_INTR_DTO) {
            cs = CS_DATA_TO_ERR;
        }
        if(sts & LS10XX_INTR_DCRC) {
            cs = CS_DATA_CRC_ERR;
        }
        if(sts & LS10XX_INTR_DEB) {
            cs = CS_DATA_END_ERR;
        }
        if(sts & LS10XX_INTR_CTO) {
            cs = CS_CMD_TO_ERR;
        }
        if(sts & LS10XX_INTR_CCRC) {
            cs = CS_CMD_CRC_ERR;
        }
        if(sts & LS10XX_INTR_CEB) {
            cs = CS_CMD_END_ERR;
        }
        if(sts & LS10XX_INTR_CIE) {
            cs = CS_CMD_IDX_ERR;
        }
        if(sts & LS10XX_INTR_DMAE) {
            cs = CS_DATA_TO_ERR;
        }
        if(sts & LS10XX_INTR_ACE) {
            cs = CS_DATA_TO_ERR;
        }
        if(!cs) {
            cs = CS_CMD_CMP_ERR;
...
static int ls10xx_sdhcx_abort(__attribute__((unused)) sdio_hc_t *hc, __attribute__((unused)) sdio_cmd_t *cmd)
{
    return(EOK);
}
...
static int ls10xx_sdhcx_pwr(sdio_hc_t *hc, int vdd)
{
    ...
    if ( (hc->vdd == (uint32_t)vdd) || (hc->vdd && vdd) ) {
        return (EOK);
    }
...
static int ls10xx_sdhcx_clk(sdio_hc_t *hc, int clk)
{
    ls10xx_hc_t *sdhc  = (ls10xx_hc_t *) hc->cs_hdl;
    ...
    while (((base_clk / pre_div / 16) > (uint32_t)clk) && (pre_div < 256)) {
        pre_div *= 2;
    }

    while ((base_clk / pre_div / div > (uint32_t)clk) && (div < 16)) {
        div++;
    }

    sdhc->clk_div = pre_div * div;
...
{
    if((hc->version < LS10XX_HOSTVER_SPECVER_3) || (hc->signal_voltage == (uint32_t)signal_voltage)) {
        return(EOK);
    }
...
static int ls10xx_send_tune_cmd(sdio_hc_t *hc, int op)
{
    struct sdio_cmd  cmd;
...
#ifdef LS10XX_ESDHC_ERRATA_A008171_WA
static int ls10xx_sdhcx_tune_errata_a008171_wa(sdio_hc_t *hc, int op, int hw_tuning_result)
{
    ls10xx_hc_t *sdhc  = (ls10xx_hc_t *) hc->cs_hdl;
    int          window_size;
    uint8_t      wndw_start_ptr, wndw_end_ptr;
    int          status = EOK;
    uint32_t     data;

    /* LS1043ACE_Rev10.pdf (LS1043A Chip Errata, Rev. 10, 02/2020)
     * A-008171: eSDHC transactions may fail in tuning modes of operation when the input data
     * window is close to 1 UI during tuning.
     */

    LS10XX_LOG(hc, _SLOG_INFO, "Tuning workaround for errata A-008171");

    /* 1. Read the TBCTL register. Write TBCTL[20:23] =4’h8 and wait for 1 ms. */
    data = ls10xx_in32(hc, LS10XX_TUNE_CTRL);
    data &= ~(0xF00);
    data |= 0x800;
    ls10xx_out32(hc, LS10XX_TUNE_CTRL, data);
    delay(1);

    /* 2. Read the TBCTL register and rewrite again. Wait for 1 ms. */
    data = ls10xx_in32(hc, LS10XX_TUNE_CTRL);
    ls10xx_out32(hc, LS10XX_TUNE_CTRL, data);
    delay(1);

    /* 3. Read the TBSTAT register twice. */
    ls10xx_in32(hc, LS10XX_TUNE_STAT);
    data = ls10xx_in32(hc, LS10XX_TUNE_STAT);

    /* 4. Reset data lines by setting ESDHCCTL[RSTD] bit. Poll ESDHCCTL[RSTD]. Proceed to
     *    next step when ESDHCCTL[RSTD]=0.
     */
    ls10xx_out32(hc, LS10XX_SYSCTL, ls10xx_in32(hc, LS10XX_SYSCTL) | LS10XX_SYSCTL_SRD);
    ls10xx_waitmask(hc, LS10XX_SYSCTL, LS10XX_SYSCTL_SRD, 0, 10000);

    /* 5. Write 32’hFFFF_FFFF to IRQSTAT register. */
    ls10xx_out32(hc, LS10XX_TUNE_STAT, 0xFFFFFFFF);

    /*
     * 6.
     *   a. If TBSTAT[16:23]-TBSTAT[24:31] > (4*DIV_RATIO) + 2 or
     *      TBSTAT[24:31] - TBSTAT[16:23] > (4*DIV_RATIO) + 2, then program
     *      TBPTR[TB_WNDW_END_PTR] = 4*DIV_RATIO (that is, four times the division
     *      ratio of SD_CLK) and program TBPTR[TB_WNDW_START_PTR] = (8*DIV_RATIO - 1).
     *   b. Else If in case of tuning error, program TBPTR[TB_WNDW_END_PTR] = 3*DIV_RATIO
     *      and program TBPTR[TB_WNDW_START_PTR] = 5*DIV_RATIO.
     *   c. Else in case of successful tuning, go to step 15.
     */
    window_size = ((data >> 8) & 0xff) - (data & 0xff); /* Tuning end ptr - start ptr */
    if (window_size < 0) {
        window_size *= -1;
    }

    LS10XX_LOG(hc, _SLOG_DEBUG1, "window_size %d clk_div %d", window_size, sdhc->clk_div);

    if ( window_size > (4 * sdhc->clk_div + 2) ) {
        wndw_start_ptr = 8 * sdhc->clk_div - 1;
        wndw_end_ptr = 4 * sdhc->clk_div;
    } else if (hw_tuning_result != EOK) {
        wndw_start_ptr = 5 * sdhc->clk_div;
        wndw_end_ptr = 3 * sdhc->clk_div;
    } else if (hw_tuning_result == EOK) {
        LS10XX_LOG(hc, _SLOG_INFO, "Tuning data window pointers are OK. No need for SW tuning");
        goto cleanup;
    }

    LS10XX_LOG(hc, _SLOG_DEBUG1, "wndw_start_ptr %d wndw_end_ptr %d", wndw_start_ptr, wndw_end_ptr);

    ls10xx_out32(hc, LS10XX_TUNE_PTR,
            ((wndw_start_ptr & LS10XX_TUNE_PTR_START_MASK) << LS10XX_TUNE_PTR_START_SHIFT) |
            ((wndw_end_ptr & LS10XX_TUNE_PTR_END_MASK) << LS10XX_TUNE_PTR_END_SHIFT));

    /* 7. Program the software tuning mode by setting TBCTL[TB_MODE] = 2'h3. */
    LS10XX_LOG(hc, _SLOG_INFO, "Trying SW tuning...");
    data = ls10xx_in32(hc, LS10XX_TUNE_CTRL);
    data &= ~LS10XX_TUNE_CTRL_MODE_MASK;
    data |= LS10XX_TUNE_CTRL_MODE_SW_TUNE;
    ls10xx_out32(hc, LS10XX_TUNE_CTRL, data);

    /* 8. Set SYSCTL2[EXTN] bit and wait for 1us. */
    data = ls10xx_in32(hc, LS10XX_AC12_SYSCTL2);
    data |= LS10XX_SYSCTL2_EXTN;
    ls10xx_out32(hc, LS10XX_AC12_SYSCTL2, data);
    nanospin_ns(1000);

    /* 9. Set SYSCTL2[SMPCLKSEL] bit. */
    data |= LS10XX_SYSCTL2_SMPCLKSEL;
    ls10xx_out32(hc, LS10XX_AC12_SYSCTL2, data);

    /*
     * 10. Issue SEND_TUNING_BLK command (CMD19 for SD, CMD21 for MMC).
     * 11. Wait for IRQSTAT[BRR], the buffer read ready to be set. If this bit remains 0, it
     *     indicates that SD/MMC device did not respond to SEND_TUNING_BLK. The tuning process
     *     cannot continue further.
     * 12. Clear IRQSTAT[BRR].
     */
    ...
    /* 13. Poll SYSCTL2[EXTN]. Proceed to next step whenSYSCTL2[EXTN]=0. */
    if ((status = ls10xx_waitmask(hc, LS10XX_AC12_SYSCTL2, LS10XX_SYSCTL2_EXTN, 0, 100000)) != EOK){
        goto cleanup;
    }

    /*
     * 14. Check SYSCTL2[SMPCLKSEL], Sampling Clock Select.
     *     a. If SMPCLKSEL=1, then the tuning procedure succeeded.
     *     b. Else if SMPCLKSEL=0 the tuning procedure failed. Change the SD_CLK frequency
     *        and restart hardware tuning.
     */
    data = ls10xx_in32(hc, LS10XX_AC12_SYSCTL2);
    if ( (data & LS10XX_SYSCTL2_SMPCLKSEL) == 0) {
        status = EIO;
    }

    LS10XX_LOG(hc, _SLOG_INFO, "SW tuning %s", (status == EOK) ? "succeeded" : "failed");

    /* 15. Workaround is complete (exit). */
cleanup:
    return status;
}
#endif
...
static int ls10xx_sdhcx_tune(sdio_hc_t *hc, int op)
{
     ...
     * Set Tuning Mode 3: Software Timer, and Auto Re-tuning during data transfer
     */
    LS10XX_LOG(hc, _SLOG_INFO, "Trying HW tuning (mode 3)...");
    data = ls10xx_in32(hc, LS10XX_TUNE_CTRL);
    data &= ~LS10XX_TUNE_CTRL_MODE_MASK;
    data |= (LS10XX_TUNE_CTRL_MODE3_SWT_AUTO | LS10XX_TUNE_CTRL_ENABLE);
    ls10xx_out32(hc, LS10XX_TUNE_CTRL, data);

    if ((status = ls10xx_clock_set(hc, 1)) != EOK) {
        data &= ~LS10XX_TUNE_CTRL_ENABLE;
        ls10xx_out32(hc, LS10XX_TUNE_CTRL, data);
        return status;
    }
    ...
    while (retries++ < LS10XX_TUNING_RETRIES) {
        ...
        status = EIO;
        data = ls10xx_in32(hc, LS10XX_AC12_SYSCTL2);
        if ((data & LS10XX_SYSCTL2_SMPCLKSEL) && ((data & LS10XX_SYSCTL2_EXTN) == 0)) {
            status = EOK;
            break;
        }
    }

    LS10XX_LOG(hc, _SLOG_INFO, "HW tuning %s retries %d",
                (status == EOK) ? "succeeded" : "failed", retries);

#ifdef LS10XX_ESDHC_ERRATA_A008171_WA
    /* eSDHC may indicate tuning was successful even when not tuned properly. */
    status = ls10xx_sdhcx_tune_errata_a008171_wa(hc, op, status);
#endif

    /* Clear the excute tuning bit, in case of timeout and is still enabled. */
    ls10xx_out32(hc, LS10XX_AC12_SYSCTL2,
            ls10xx_in32(hc, LS10XX_AC12_SYSCTL2) & ~LS10XX_SYSCTL2_EXTN);
    ...
    LS10XX_LOG(hc, _SLOG_INFO, "Tuning %s.", (status == EOK) ? "succeeded" : "failed");
    return(status);
}
...
int ls10xx_sdhcx_dinit(sdio_hc_t *hc)
{
    ...
    if(sdhc->adma) {
        sdio_free(sdhc->adma, sizeof(ls10xx_adma32_t) * ADMA_DESC_MAX);
    }
...
static void ls10xx_capabilities_set(sdio_hc_t *hc) {
    uint32_t     cap;
    ls10xx_hc_t *sdhc = (ls10xx_hc_t *) hc->cs_hdl;
    ...
    hc->caps |= HC_CAP_BW4 | HC_CAP_CD_WP;
...
static sdio_hc_entry_t ls10xx_sdhcx_hc_entry =	{	.nentries			= SDIO_HC_ENTRY_NFUNCS,
													.dinit				= ls10xx_sdhcx_dinit,
													.pm					= NULL,
													.cmd				= ls10xx_sdhcx_cmd,
													.abort				= ls10xx_sdhcx_abort,
													.event				= ls10xx_sdhcx_event,
													.cd					= ls10xx_sdhcx_cd,
													.pwr				= ls10xx_sdhcx_pwr,
													.clk				= ls10xx_sdhcx_clk,
													.bus_mode			= ls10xx_sdhcx_bus_mode,
													.bus_width			= ls10xx_sdhcx_bus_width,
													.timing				= ls10xx_sdhcx_timing,
													.signal_voltage		= ls10xx_sdhcx_signal_voltage,
													.drv_type			= NULL,
													.driver_strength	= NULL,
													.tune				= ls10xx_sdhcx_tune,
													.preset				= NULL
												};
...
int ls10xx_sdhcx_init(sdio_hc_t *hc)
{
    ...
    if (hc->caps & HC_CAP_DMA) {
        if(hc->version >= LS10XX_HOSTVER_SPECVER_3) {
            ...
            if((sdhc->adma = sdio_alloc(sizeof(ls10xx_adma32_t) * ADMA_DESC_MAX)) == NULL) {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/hardware/devb/sdmmc/sdiodi/hc/ls10xx.c $ $Rev: 970776 $")
#endif
