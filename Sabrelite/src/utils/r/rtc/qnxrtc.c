...
#ifdef RTCALARM
#define RTCALARM_OPTIONS "a:A:"
#else
#define RTCALARM_OPTIONS
#endif
...
#ifdef RTCALARM
#define ALARM_STR_LEN 64
static char	alarm_time_str[ALARM_STR_LEN] = {0};
static char	alarm_enable_str[ALARM_STR_LEN] = {0};
static char	enable_alarm = 0;
static char	set_alarm = 0;
#endif
...
static struct rtc_desc clocks[] ={
	...
	{ "isl1208",		init_isl1208, get_isl1208, set_isl1208 },
	...
	{ "m41t00",		init_m41t00, get_m41t00, set_m41t00 },
	...
	{ "pcf2127at",		init_pcf2127at, get_pcf2127at, set_pcf2127at},
	{ "rv3028",		init_rv3028, get_rv3028, set_rv3028},
	{ .name = "rx6110",	.init = init_rx6110, .get = get_rx6110, .set = set_rx6110},
	{ "tps65910",       init_tps65910, get_tps65910, set_tps65910 },
#if (defined __aarch64__)
	{ "primecell",		init_primecell, get_primecell, set_primecell },
	{ "pmqcom",		init_pmqcom, get_pmqcom, set_pmqcom },
	{ "mt2712",		init_mt2712, get_mt2712, set_mt2712 },
	{ .name = "mcp7941x",	.init = init_mcp7941x, .get = get_mcp7941x, .set = set_mcp7941x },
	{ .name = "mx8msrtc",	.init = init_mx8msrtc, .get = get_mx8msrtc, .set = set_mx8msrtc},
#if (defined VARIANT_imx8x)
	{ "mx8sc",		init_mx8sc, get_mx8sc, set_mx8sc },
#endif
#if (defined VARIANT_imx9)
	{ .name = "mx93bbnsm",  .init = init_mx93bbnsm, .get = get_mx93bbnsm, .set = set_mx93bbnsm},
#endif
	{ "xzynq",		init_xzynq, get_xzynq, set_xzynq },
#endif
	{ NULL,			NULL }
};

#ifdef RTCALARM
static struct rtc_alarm_desc rtc_alarm[] ={
        { "ds3232", set_alarm_ds3232, enable_alarm_ds3232},
#if (defined __aarch64__)
	{ "pmqcom", set_alarm_pmqcom, enable_alarm_pmqcom },
#endif
#if (defined VARIANT_imx8x)
	{ "mx8sc",	set_alarm_mx8sc, enable_alarm_mx8sc },
#endif
	{ NULL, NULL }
};
#endif
...
main(int argc, char *argv[]) {
	...
	chip.optstr = NULL;
	while ((opt=getopt(argc,argv,"b:ls" SLOWADJUST_OPTIONS RTCALARM_OPTIONS VERBOSE_OPTIONS))!=-1) {
            switch(opt) {
		case 'b':
...
#ifdef RTCALARM
		case 'a':
                        src_len = strlcpy(alarm_enable_str, optarg, sizeof(alarm_enable_str));
                        if (src_len >= sizeof(alarm_enable_str)) {
                                fprintf(stderr,"rtc: The <alarm_enable> option to '-a' is too long (max length %u)\n", ALARM_STR_LEN);
                                error++;
			} else {
                                enable_alarm = 1;
			}
		...
		case 'A':
                        src_len = strlcpy(alarm_time_str, optarg, sizeof(alarm_time_str));
                        if (src_len >= sizeof(alarm_time_str)) {
                                fprintf(stderr,"rtc: The <alarm_time> option to '-A' is too long (max length %u)\n", ALARM_STR_LEN);
                                error++;
                        } else {
                                set_alarm = 1;
			}
	...
	for( ;; ) {
		...
		if(strcmp(clocks[clk].name, given_name) == 0) {
			...
			/*
				Specified 'hw' for the name. Figure out what hardware
				is on the system.
			*/
...
#ifdef RTCALARM
	if (set_alarm || enable_alarm) {
		alarm_idx = 0;
		for( ;; ) {
			if (rtc_alarm[alarm_idx].name == NULL) {
				fprintf(stderr, "rtc: setting/enabling alarm is not supported for this clock type.\n");
				set_alarm = 0;
                                enable_alarm = 0;
				exit(EXIT_FAILURE);
			}
			if(strcmp(rtc_alarm[alarm_idx].name, given_name) == 0) {
				if (((rtc_alarm[alarm_idx].set_alarm != NULL) || !set_alarm) &&
					((rtc_alarm[alarm_idx].enable_alarm != NULL) || !enable_alarm)) break;
	...
	/* Command line options overwrites optstr from syspage */
	if(argv[optind+1] && argv[optind+1][0])
		size = clocks[clk].init(&chip, &argv[optind+1]);
	else
		size = clocks[clk].init(&chip, &chip.optstr);
...
#ifdef RTCALARM
	if (set_alarm) {
                if (rtc_alarm[alarm_idx].set_alarm(alarm_time_str) != EOK) {
			exit(EXIT_FAILURE);
		}
	}

	if (enable_alarm) {
		if (rtc_alarm[alarm_idx].enable_alarm(alarm_enable_str) != EOK) {
			exit(EXIT_FAILURE);
		}
	}

	if (set_alarm || enable_alarm) {
		/* Don't handle other options if rtc alarm has been set/enabled. */
		exit(EXIT_SUCCESS);
	}
#endif
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/utils/r/rtc/qnxrtc.c $ $Rev: 986017 $")
#endif
