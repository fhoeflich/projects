...
use_hwinfo(struct chip_loc *chip) {
	...
	loc  = hwi_find_tag(off, 1, HWI_TAG_NAME_regname);
	if(loc != HWI_NULL_OFF) {
		tag = hwi_off2tag(loc);
		chip->century_reg = tag->regname.offset;
	}

	/* command line argument string to the RTC utility */
	loc = hwi_find_tag(off, 1, HWI_TAG_NAME_optstr);
	if(loc != HWI_NULL_OFF) {
		tag = hwi_off2tag(loc );
		chip->optstr = strdup( __hwi_find_string( tag->optstr.string ) );
	}
	return(name);
}
...
load_external_clock(const char *given_name, struct rtc_desc *clk) {
	...
	len = snprintf(buff, sizeof(buff), "rtc-%s.so", given_name);
	if(len >= sizeof(buff))	{
		fprintf(stderr, "Invalid clock type\n");
		return(-1);
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/utils/r/rtc/nto/support.c $ $Rev: 725908 $")
#endif
