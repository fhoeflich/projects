...
#ifdef RTCALARM
int
RTCFUNC(set_alarm,pmqcom)(const char* alarm_time_str) {
	...
	int ret = EOK;
        ...
        time = (uint32_t)strtoul (alarm_time_str, 0, 0);
...
int
RTCFUNC(enable_alarm,pmqcom)(const char* alarm_enable_str) {
	...
	int                 ret = EOK;
        ...
        /* String format is "<enable>". */
        cmd = atoi(alarm_enable_str)?1:0;
        #ifdef VERBOSE_SUPPORTED
        if (verbose) {
            fprintf(stderr, "rtc: %s rtc alarm\n", (cmd)?"enabling":"disabling");
        }
        #endif
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/utils/r/rtc/nto/aarch64/clk_pmqcom.c $ $Rev: 982741 $")
#endif
