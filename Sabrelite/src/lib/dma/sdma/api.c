...
#ifdef SDMA_CORE_CLOCK_EQUAL
static uint8_t   core_ratio = SDMA_CORE_CLOCK_EQUAL;
#else
static uint8_t   core_ratio = 0;
#endif
...
static char * sdma_opts[] = {
    ...
    "regbase",
    "attach_process",
    "ratio",    /**> SDMA core clock ratio. 1 means core clock is equal to bus clock */
    NULL
};
...
static char * channel_create_opts[] = {
    ...
    "regen",
    "contloop",
    "runtime_ctx_load", /**> Enables channel ctx load during xfer_setup(). Used to provide watermark value in xfer_setup()
                             and not during channel_attach(). Used by audio driver. */
    NULL
};
...
static sdma_chan_t * chan_create(unsigned ch_num) {
    sdma_chan_t * chan_ptr;
    int status, below4G_fd;
    off64_t paddr64;
    ...
    /* Open typed memory for below4G mappings */
    below4G_fd = posix_typed_mem_open( "/memory/below4G/ram/sysram", O_RDWR, POSIX_TYPED_MEM_ALLOCATE_CONTIG );
    if (below4G_fd < 0)
    {
        perror("Failed to open typed memory");
        goto fail2;
    }
    ...
    status = mem_offset64(   (void*) chan_ptr->bd_ptr,
     ...
     // Note: Our memory is from below4G typed memory region so we know it fits in 32bits
...
static void chan_destroy(sdma_chan_t * chan_ptr) {
    ...
    if ( sdmasync_is_first_process()  ) {
        ...
        if (core_ratio) {
            out32(sdma_base + SDMA_CONFIG,
                 (in32(sdma_base + SDMA_CONFIG) & ~CONFIG_CSM_MSK) | CONFIG_ACR);
        } else {
            out32(sdma_base + SDMA_CONFIG,
...
parse_init_options(const char *options)
{
    ...
    if(hwi_off != HWI_NULL_OFF) {
        ...
        if(tag) {
                ...
                sdma_irq = (int)hwitag_find_ivec(hwi_off, NULL);
        }
    }
    ...
    while (temp_ptr && *temp_ptr != '\0') {
        ...
        switch (opt) {
            case 0:
                sdma_irq = strtoul(value, 0, 0);
                break;
            case 1:
                regphys = strtoul(value, 0, 0);
                break;
            case 2:
                init_flags = strtoul(value,0,0);
                if (init_flags != 0)
                    init_flags |= DMA_ATTACH_PROCESS;
                break;
            case 3:
                core_ratio = (uint8_t)strtoul(value, NULL, 0);
                break;
            default:
                return -1;
        }
...
static int parse_channel_options(sdma_chan_t * chan_ptr,const char *options) {
    char    *value;
    int     opt;
    char    *temp_ptr = NULL;
    char    *temp_ptr_head = NULL;
...
static void callback_reenable_descr(unsigned ch_num) {
    int i;
...
#if __PTR_BITS__ > 32
    // bd_ptr has NOCACHE flag thus call dsb explicitly
    dsb();
#endif
}
...
static int sdma_init(const char *options) {
    ...
    if (n_users_in_process == 1) {
        ...
        if ( sdmairq_init(sdma_irq, init_flags) != 0 ) {
            goto fail3;
        }
    ...
    const struct sigevent *event, unsigned *channel, int prio, unsigned flags)
{
    ...
    req.start = SDMA_CH_LO + RSRCDBMGR_DMA_CH_START;
    req.end = SDMA_CH_HI + RSRCDBMGR_DMA_CH_START;
    req.length = 1;
    req.flags = RSRCDBMGR_DMA_CHANNEL | RSRCDBMGR_FLAG_RANGE;
    if (rsrcdbmgr_attach(&req, 1) != EOK) {
        goto fail1;
    }
    ch_num = (unsigned)(req.start - RSRCDBMGR_DMA_CH_START);
    ...
    if (!chan_ptr->runtime_ctx) {
        // configure and load the SDMA context into private SDMA memspace
...
fail2:
    req.length = 1;
    req.start = ch_num + RSRCDBMGR_DMA_CH_START;
    req.end = req.start;
...
sdma_channel_release(void * handle) {
    ...
    req.length = 1;
    req.start = req.end = chan_ptr->ch_num + RSRCDBMGR_DMA_CH_START;
    req.flags = RSRCDBMGR_DMA_CHANNEL;
    rsrcdbmgr_detach(&req, 1);
...
sdma_setup_xfer(void *handle, const dma_transfer_t *tinfo) {
    ...
    int i;
    if (chan_ptr->runtime_ctx) {
        if ((!chan_ptr->runtime_ctx_loaded) || (chan_ptr->watermark != tinfo->xfer_bytes)) {
            chan_ptr->watermark = tinfo->xfer_bytes;
            // Configure and load the SDMA context into private SDMA memspace.
            sdmacmd_ctx_config(chan_ptr);
            if (sdmacmd_ctx_load(chan_ptr) != 0) {
                return -1;
            }
            chan_ptr->runtime_ctx_loaded = 1;
        }
    }
    //////////////////////////////////////////////
    ...
    case SDMA_CHTYPE_SSISH_2_MCU:
    case SDMA_CHTYPE_ZQSPI_2_MCU:
...
#endif
#else
static int sdma_alloc_buffer(void *const handle, dma_addr_t *addr, const unsigned size, const unsigned flags)
{
    int below4G_fd;
    ...
    /* Open typed memory for below4G mappings */
    below4G_fd = posix_typed_mem_open( "below4G&sysram", O_RDWR, POSIX_TYPED_MEM_ALLOCATE_CONTIG );
    if (below4G_fd < 0) {
        goto fail1;
    }
    addr->vaddr = mmap(NULL, size, prot_flags, MAP_SHARED, below4G_fd, 0);
    close(below4G_fd);

    if (addr->vaddr == MAP_FAILED) {
        goto fail1;
    }
    if (mem_offset64(addr->vaddr, NOFD, 1, &addr->paddr, NULL) != EOK) {
        goto fail2;
    }
    addr->len = size;
    return 0;
 ...
 */
static void sdma_free_buffer(void *const handle, dma_addr_t *addr)
{
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/lib/dma/sdma/api.c $ $Rev: 972859 $")
#endif
