/*
 * $QNXLicenseC:
 * Copyright 2009, 2022 BlackBerry Limited.
...
static int shared_mem_create() {
...
#else
    int fd_dma;
    sdma_dma_mem_t *dma_mem_ptr;
    size_t dma_mem_size;
#endif
    pthread_mutexattr_t mutex_attr;

    fd = shm_open(SDMA_MUTEX_PATH, O_RDWR | O_CREAT | O_EXCL, 0666);
    ...
    if (fd >= 0) {
         ...
         * process terminates.
         */
        if ( (paddr64 > ((uint64_t)INT64_MAX - sizeof(sdma_shmem_t))) ||
...
#else
        fd_dma = posix_typed_mem_open("/memory/dma", O_RDWR, POSIX_TYPED_MEM_ALLOCATE_CONTIG);
        if (fd_dma == NOFD) {
            status = errno;
            perror("shared_mem_create() cannot open /memory/dma region\n");
            goto fail3;
        }
        dma_mem_size = sizeof(sdma_dma_mem_t);
        /* Temporary mmap CCB into our address space to get phys. address */
        dma_mem_ptr = mmap(NULL, dma_mem_size,
                           PROT_READ | PROT_WRITE | PROT_NOCACHE,
                           MAP_SHARED | MAP_NOINIT, fd_dma, (off_t)(dma_mem_size * SDMA_INSTANCE));
        status = errno;
        close(fd_dma);
        if (dma_mem_ptr == MAP_FAILED) {
            perror("shared_mem_create() /memory/dma mmap failed\n");
            goto fail3;
        }
        /* Clear CCB */
        memset((void*)dma_mem_ptr, 0, dma_mem_size);
        /* Get the physical memory address of the CCB structure. */
        status = mem_offset64((void*)dma_mem_ptr, NOFD, dma_mem_size, &paddr64, NULL);
        if (status != 0) {
            status = errno;
            perror("shared_mem_create() Couldn't get physical address of ccb memory\n");
            goto fail4;
        }

        if ( ((uint64_t)paddr64 > (INT64_MAX - dma_mem_size)) ||
             (((uint64_t)paddr64 + dma_mem_size) >= ARM_LPAE_4G_BOUNDARY) )
        {
            status = ENOTSUP;
            perror("Wrong dma memory address. Check /memory/dma region\n");
            goto fail4;
        }
#endif
...
#else
        //save the physical memory address of the dma mem structure
        shmem_ptr->paddr = (sdma_dma_mem_t *) paddr64;

        // Save the physical memmory address of the Command channel buffer descriptor
        dma_mem_ptr->ccb[SDMA_CMD_CH].base_bd_paddr = (uint32_t)(uintptr_t) &shmem_ptr->paddr->cmd_chn_bd;
        dma_mem_ptr->ccb[SDMA_CMD_CH].current_bd_paddr = dma_mem_ptr->ccb[SDMA_CMD_CH].base_bd_paddr;
        /* Unmap and close object.  This is mapped again in sdmasync_init() */
        munmap((void*)dma_mem_ptr, dma_mem_size);
#endif
...
void ctor(void) {
    ...
    if (shared_mem_create() == EOK) {
        ...
        ralloc.start    = SDMA_CH_LO + RSRCDBMGR_DMA_CH_START;
        ralloc.end      = SDMA_CH_HI + RSRCDBMGR_DMA_CH_START;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/lib/dma/sdma/init.c $ $Rev: 972795 $")
#endif
