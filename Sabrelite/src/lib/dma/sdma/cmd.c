...
static int sdma_wait_complete(void)
{
    struct _pulse    pulse;
    const uint64_t   timeout = 2 * SDMA_CMD_TIMOUT_NS;
    int              retval;

    TimerTimeout(CLOCK_REALTIME, _NTO_TIMEOUT_RECEIVE, NULL, &timeout , NULL);
    while (1) {
        retval = MsgReceivePulse(cmd_chid, &pulse, sizeof(pulse), NULL);
        if (retval != 0) {
            perror("MsgReceivePulse() timedout");
            break;
        }

        if (pulse.code == SDMA_CMD_COMPLETE_PULSE_CODE) {
            retval = 0;
            break;
        }
    }
...
static int sdmaram_script_load() {
    uint16_t        *script;
    int             status;
    int             below4G_fd;
    ...
    // map physical memory for holding SDMA script code from the below4G typed memory pool
...
#if __PTR_BITS__ > 32
    // script has NOCACHE flag thus call dsb()
    dsb();
#endif
    ...
    cmd_bd_ptr[0].buf_paddr = (uint32_t) script_paddr;
    cmd_bd_ptr[0].ext_buf_paddr = scriptinfo.ram_microcode_info.addr;
    CACHE_FLUSH(    &cinfo,
                    (void *)cmd_bd_ptr,
                    ccb_ptr[SDMA_CMD_CH].base_bd_paddr,
                    sizeof(sdma_bd_t)    );
...
fail3:
    munmap(script, scriptinfo.ram_microcode_info.size);
fail2:
    close(below4G_fd);
fail1:
    return -1;
...
// Initialize the command channel.  The command channel lives at channel 0 and is
...
int sdmacmd_cmdch_create(void) {

	cmd_bd_ptr = sdmasync_cmdbd_ptr_get();

    if ( sdmasync_is_first_process() ) {
        // Flush the ccb entry for the command channel
        CACHE_FLUSH(    &cinfo,
...
void sdmacmd_cmdch_destroy(void) {
    ConnectDetach( cmd_coid );
    ChannelDestroy(cmd_chid);
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/lib/dma/sdma/cmd.c $ $Rev: 972859 $")
#endif
