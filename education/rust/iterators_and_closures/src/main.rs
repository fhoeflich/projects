...
fn check_inventory(items: Vec<Item>, product: String) -> Vec<Item> {
    // items.into_iter().filter(|i| i.color == product).collect()
...
fn main() {
	...
	// FnMut can be called multiple times if the closure is declared mut
	// FnOnce can be called once if the caller owns the closure

    // || drop(v)			  FnOnce (non-idempotent - can only drop value once)
    // |args| v.contains(arg) Fn  (idempotent - can check contents as much as you want)
    // |args| v.push(arg)	  FnMut (idempotent - iff v is mut so we can push arg into v)
    ...
    // while let v = iter.next() { // allows None to be extracted, ad infinitum
    // while let Some(v) = iter.next() {	// only allows non-None values of v
    //     println!("{:?}", v);
    // }
        ...
        // let checked = check_inventory(vec, String::from("plaid"));
